...
- Two on-prem controllers: older **ezMaster** (EOL) and modern **Fit Controller /
  EnGenius Private Cloud (EPC)**. Details: [controllers/landscape](controllers/landscape.md).
...
- `-FIT` is a firmware/feature variant for the local Fit Controller. Some non-FIT
  devices convert to their FIT sibling by [cross-flashing](firmware/cross-flashing.md).
...
[hardware/model-equivalence](hardware/model-equivalence.md).
